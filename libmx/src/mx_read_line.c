...
static int find_line(char **static_buff, char **buff, char **lineptr, char delim);
static char *cut_static_buff_before_index(char**static_buff, int index);
static int check_static(char **static_buff, char **lineptr, char delim);
static int return_static(char** lineptr, char**static_buff);
...
int mx_read_line(char **lineptr, int buf_size, char delim, const int fd) {
	...
	if ((fd <= 0) || (buf_size <= 0) || (read(fd, NULL, 0) == -1))
		return -2;
	if (check_static(&static_buff, lineptr, delim) == 1)
	...
	while ((i = read(fd, (void*)buff, buf_size)) > 0) {
		if ((mx_strchr((char*)buff, delim)) == NULL)
			...
			return find_line(&static_buff, &buff, lineptr, delim);
		}
		mx_strdel(&buff);
		if(i == 0 && static_buff != NULL)
			return return_static(lineptr, &static_buff);
		return -1;
}
...
static int find_line(char **static_buff, char **buff, char **l, char d) {
	char *tmp = NULL;
	int index = 0;
...
static int check_static(char **static_buff, char **lineptr, char delim) { 
	...
	if (index < 0)
		return 0;
	if (index == 0) {
		*static_buff = cut_static_buff_before_index(static_buff, index);
		*lineptr = mx_strdup("");
		return 1;
	}
